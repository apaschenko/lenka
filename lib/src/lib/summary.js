...
class Summary {
    ...
    validateAndBuildOptions(operation, rawOptions = {}) {
                ...
                default:
                    if (optionName === 'actions' && operation === 'combine') {
                        ...
                        finalOptions.actions = optionValue.map((rawAction) => {
                            return new actions_1.FinalAction(rawAction);
                        });
                    }
                    else {
        ...
        if (operation === 'combine') {
            finalOptions.actions.push(new actions_1.FinalAction({ coverage: 'all', actor: actions_1.PredefinedActorFunctions.replace.actor }));
        }
        return finalOptions;
    }
    initRoots(rawData) {
