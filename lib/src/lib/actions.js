...
exports.PredefinedActorFunctions = {
    ...
    diff: {
        ...
        actor: function (combineParams, actorParams) {
            const { bases, scheme } = combineParams;
            const { byProperties, byKeys, byValues, byArrayKeys, keysPropsMix, valuesFromProps, valuesFromKeys, } = actorParams;
...
class FinalAction {
    constructor(rawAction) {
        ...
        if (keys.length !== 2 || !keys.every((key) => { return ActionKeys.has(key); })) {
            this.throwError();
        }
