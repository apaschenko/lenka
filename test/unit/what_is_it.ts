...
const fixtures = [
  ...
  {
    value: function myFunction(a): any {
      return a;
    },
    type: 'function',
  },
  ...
  {
    value: async function myAsyncFunction() {
      await new Promise((resolve) => setTimeout(() => resolve(1), 10));
    },
    type: 'asyncFunction',
  },
  {
    value: function* myGenerator() {
      yield 1;
    },
  ...
  { value: NaN, type: ['number', 'nan', 'number', 'nan'] },
  {
    ...
    type: ['number', 'number', 'infinity', 'infinity'],
  },
  {
...
describe('====== typeOf ======', function () {
  for (let i = 0; i < 4; i++) {
    const nan = i % 2 ? true : false;
    const infinity = i > 1;

    describe(`--- nan: ${nan ? 'true' : 'false'}, infinity: ${
      infinity ? 'true' : 'false'
      ...
      for (const [index, item] of fixtures.entries()) {
        it(`test ${index}`, function () {
          expect(
            whatIsIt(item.value, { nan: i % 2 ? true : false, infinity: i > 1 })
